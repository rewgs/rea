...
function toggle_mark_track(all_tracks, mark)
    -- local num_tracks = reaper.CountSelectedTracks(0)
    for i = 0, #all_tracks - 1 do
        local current_track = all_tracks[i]
        reaper.ShowConsoleMsg(current_track.name)
    ...
    --         local marked = is_marked(selected_track_name, mark)
    --         if marked then
